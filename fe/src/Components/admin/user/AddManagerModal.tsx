import { HouseholdName, IInsertUserType } from '@/types/userType';
...
export interface IAddManagerProps {
            ...
            <div className="bg-white p-4 rounded-lg w-1/3 shadow-md">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className='flex flex-col'>
                        ...
                        <label htmlFor="householdName">Hộ kinh doanh</label>
                        <select {...register('householdId')} name="householdName" id="householdName" className='border border-gray-300 rounded-lg p-2 mb-5'>
                            {householdName?.map((item) => (
                                <option key={item.householdId} value={item.householdId}>{item.householdName}</option>
                            ))}
                        </select>
                    </div>
