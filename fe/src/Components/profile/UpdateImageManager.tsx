...
export interface IUpdateProfileManagerProps {
    isEditAvatar: boolean;
    setIsEditAvatar: React.Dispatch<React.SetStateAction<boolean>>;
    avatar: string;
}

export default function UpdateProfileManager({
    setIsEditAvatar,
    isEditAvatar,
    avatar,
}: IUpdateProfileManagerProps) {
    ...
    const onSubmit = async () => {
        if (imageFile) {
            try {
                setLoading(true);
                await authApi.updateManagerAvatar(imageFile);
