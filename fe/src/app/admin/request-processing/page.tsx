'use client'
import RequestData from '@/Components/admin/request-processing/RequestData';
...
export default function RequestProcessing(props: IRequestProcessingProps) {
        ...
        (e: React.ChangeEvent<HTMLInputElement> | string) => {
            ...
            if (searchValue === "") {
                ...
                const searchResult = displayData?.filter((request) => {
                    return (
                        request.householdName.toLowerCase().includes(searchValue.toLowerCase()) ||
    ...
    React.useEffect(() => {
        setLoading(true);
        requestProcessingApi.getAllRequest().then((res) => {
                        ...
                        <tbody>
                            {searchData && (
                                currentSearchData?.map((request, index) => (
                                    <RequestData key={index} request={request} change={changeData} setChange={setChangeData} />
                                ))
                            )}
