...
import { IGetRoomInformationToAdd, Passenger, responseRoomInformation } from '@/types/dashboardType';
import { Breadcrumb, Input, Tag, message } from 'antd';
import { useRouter } from 'next/navigation';
import * as React from 'react';
import { SubmitHandler, useForm } from 'react-hook-form';

export interface IAddRoomDormProps {
}

export default function AddRoomDorm(props: IAddRoomDormProps) {
    const [selectedOption, setSelectedOption] = React.useState('');
    const [selectedOptionRoom, setSelectedOptionRoom] = React.useState('');
    const [roomDormData, setRoomDormData] = React.useState<IGetRoomInformationToAdd>();
    ...
    React.useEffect(() => {
        setLoading(true);
        dormApi.getRoomDormInformationToAdd().then((res) => {
            setRoomDormData(res);
    ...
    console.log('ádsadsd' + roomDormData);
    ...
    const onSubmit: SubmitHandler<responseRoomInformation> = async (data) => {
        const selectedHomestay = roomDormData?.homestay.find((items) => items.homestayCode === selectedOption);
        if (selectedHomestay) {
            const { id } = selectedHomestay;
            data.homestayId = id;
        }

        const selectedRoomType = roomDormData?.roomType.find((items) => items.roomTypeName === selectedOptionRoom);
        if (selectedRoomType) {
        ...
        try {
            setLoading(true);
            await dormApi.addRoomDorm(res);
            const updateData = await dormApi.getRoomDormInformationToAdd();
            setRoomDormData(updateData);
            message.success('Thêm phòng dorm thành công');
            router.push('/manager/room-dorm');
        } catch (err) {
            console.log(err);
    ...
    const breadcrumbItems = [
            ...
            title: (
                <>
                    <button style={{ display: 'flex', alignItems: 'center' }} className='hover:font-semibold text-black'>
                        ...
                        <span className='pl-2'>Phòng dorm</span>
                    </button>
                </>
            ),
        },
        {
            title: <div style={{ display: 'flex', alignItems: 'center' }}>
                <span className='font-bold'>Thêm phòng dorm</span>
            </div>,
        },
    ];
    return (
            ...
            <form className="w-full max-w-full pl-5" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className='md: flex mb-4'>
                    ...
                    <div className='w-2/3'>
                        <div className="relative" style={{ zIndex: 1 }}>
                            <button
                                className="bg-gray-100 text-gray-500 font-semibold py-2 px-4 rounded inline-flex items-center"
                            ...
                            {isOpen && (
                                <div className="absolute mt-1 py-2 w-40 bg-white rounded-md shadow-lg overflow-y-auto max-h-48">
                                    {roomDormData?.homestay.map((option) => (
                                        <button
                                            key={option.id}
                        ...
                        <div className="relative">
                            ...
                            {isRoomTypeOpen && (
                                <div className="absolute mt-1 py-2 w-48 bg-white rounded-md shadow-lg overflow-y-auto max-h-48">
                                    {roomDormData?.roomType.map((option) => (
                                        <button
                                            key={option.id}
                                            className="block px-4 py-2 text-gray-800 hover:bg-gray-100 w-full text-left"
                ...
                {roomDormData && (
                    <div className='md: flex mb-6'>
                        ...
                        <div className='w-2/3'>
                            {roomDormData.roomType.find((roomType) => roomType.roomTypeName === selectedOptionRoom)?.singleBed}
                        </div>
                    </div>
                )}
                ...
                {roomDormData && (
                    <div className='md: flex mb-4'>
                        ...
                        <div className='w-2/3'>
                            {roomDormData.roomType.find((roomType) => roomType.roomTypeName === selectedOptionRoom)?.doubleBed}
                        </div>
                    </div>
                )}
                <div className='md: flex mb-4'>
                    ...
                    <div className='w-2/3'>
                        <Tag color='success'>
                            {roomDormData?.roomType.find((roomType) => roomType.roomTypeName === selectedOptionRoom)?.price} VNĐ
                        </Tag>
                    </div>
                </div>
                <div className='md: flex mb-4'>
                    ...
                    <div className='w-2/3'>
                        {roomDormData?.roomType.find((roomType) => roomType.roomTypeName === selectedOptionRoom)?.capacity}
                    </div>
                </div>
                <div className='md: flex mb-4'>
