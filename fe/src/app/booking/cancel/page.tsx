...
import CancelBookingItem from '@/Components/booking/CancelBookingItem';
...
export default function CancelBooking() {
    ...
    React.useEffect(() => {
        const fetchBookingData = async () => {
            try {
                setLoading(true);
                const res = await bookingApi.getCancelBooking();
                ...
                currentData.map((item) => (
                    <>
                        <BookingList key={item.bookingCode}>
                            <CancelBookingItem item={item} />
                        </BookingList>

                    </>
                ))
            ) : (
                <p className='text-center'>Bạn chưa hủy đặt phòng lần nào</p>
            )}
