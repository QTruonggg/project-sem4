...
import hbs.booking.config.DeployConfig;
import hbs.booking.constant.DateTimeConstant;
import hbs.booking.enums.*;
import hbs.booking.mapper.BookingCreateCustomerRequestMapper;
import hbs.booking.mapper.BookingDetailRecommendResponseMapper;
import hbs.booking.mapper.RoomTypeHouseholdForBookingResponseMapper;
...
import hbs.booking.model.dto.response.booking.customer.*;
...
import hbs.booking.service.*;
import hbs.booking.util.exception.*;
...
public class BookingForCustomerServiceImpl extends BaseServiceImpl<BookingDetail, Long, BookingDetailRepository>
    ...
    private void sendEmailNotice(Customer customer, String notice) throws MessagingException {
        ...
        helper.setTo(email);
        helper.setSubject("[VHomeStay] Bạn gặp khó khăn khi thanh toán đặt phòng?");
    ...
    private void sendEmailNoticeBanUser(Customer customer, String notice) throws MessagingException {
        ...
        MimeMessageHelper helper = new MimeMessageHelper(message, true);
        helper.setTo(email);
        helper.setSubject("[VHomeStay] Tài khoản của bạn đã bị vô hiệu hóa");
        Context context = new Context();
        context.setVariable("customerName", customer.getFirstName() + " " + customer.getLastName());
        context.setVariable("notice", notice);
        context.setVariable("phoneOfVillage", phoneOfVillage);
