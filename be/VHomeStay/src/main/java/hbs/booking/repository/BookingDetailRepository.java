...
public interface BookingDetailRepository extends BaseRepository<BookingDetail, Long>{
    ...
    @Query("SELECT new hbs.booking.model.dto.response.booking.manager.BookingSummaryDto(db.householdRoomType.id, rt.roomTypeName, COUNT(*), SUM(db.subTotal)) " +
    ...
    @Query("SELECT new hbs.booking.model.dto.response.booking.customer.RoomTypeHouseholdAvailableDto(hrt.id, rt.roomTypeName, hrt.capacity, rt.singleBed, rt.doubleBed, hrt.price, hrt.isChildrenAndBed, rt.isDorm, COUNT(*)) " +
            "FROM Room r " +
    ...
    @Query("SELECT new hbs.booking.model.dto.response.bookingdetail.BookingDetailCheckInOutTodayResponseDto(h.id, h.homestayCode, rt.id, " +
