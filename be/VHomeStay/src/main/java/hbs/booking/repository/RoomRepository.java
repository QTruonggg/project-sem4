...
import hbs.booking.model.dto.response.DormInformationResponseDto;
import hbs.booking.model.dto.response.RoomInformationResponseDto;
import hbs.booking.model.dto.response.dashboard.manager.DormitoryInformationForDashboard;
import hbs.booking.model.dto.response.dashboard.manager.RoomInformationForDashboard;
import hbs.booking.model.dto.response.room.RoomSearchManagerResponseDto;
import hbs.booking.model.dto.response.room.RoomTodayManagerResponseDto;
...
public interface RoomRepository extends CrudRepository<Room, Long>, BaseRepository<Room, Long> {
    ...
    @Query("select new hbs.booking.model.dto.response.RoomInformationResponseDto(r.id, hs.homestayCode, r.roomName, " +
    ...
    @Query("select new hbs.booking.model.dto.response.DormInformationResponseDto(r.id, hs.homestayCode, hrt.price," +
            "r.roomName, r.status)" +
            "from Room r join r.homestay hs join r.householdRoomType hrt join hrt.roomType rt " +
            "join hs.household h join h.manager m join m.account a " +
...
//                    "AND b.status IN ('BOOKED', 'CHECKED_IN') " +
    ...
    @Query("SELECT new hbs.booking.model.dto.response.dashboard.manager.RoomInformationForDashboard(r.id, r.roomName, rt.roomTypeName, false, b.bookingCode, bd.checkInCustomerName) " +
            "FROM Room r " +
    ...
    @Query("SELECT new hbs.booking.model.dto.response.dashboard.manager.RoomInformationForDashboard(r.id, r.roomName, rt.roomTypeName, true, '', '') " +
            "FROM Room r " +
            "JOIN r.householdRoomType hrt " +
            "JOIN hrt.roomType rt " +
            "JOIN r.homestay hs " +
    ...
    @Query(" SELECT new hbs.booking.model.dto.response.dashboard.manager.DormitoryInformationForDashboard(r.id, r.roomName, rt.roomTypeName, 0, COUNT(ds.id)) " +
            "FROM Room r " +
            "JOIN r.dormSlots ds " +
            "JOIN r.householdRoomType hrt " +
            "JOIN hrt.roomType rt " +
