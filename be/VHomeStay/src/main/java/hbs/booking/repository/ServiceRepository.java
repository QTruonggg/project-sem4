...
public interface ServiceRepository extends BaseRepository<Service, Long> {
    @Query("SELECT new hbs.booking.model.dto.response.service.manager.ServiceDetailForAddResponse(s.id, s.serviceName) FROM Service s")
    List<ServiceDetailForAddResponse> getAllServiceForAdd();
    @Query("SELECT new hbs.booking.model.dto.response.service.customer.ServiceResponse(s.id, s.serviceName, s.image) FROM Service s where s.status != 'DELETED'")
    List<ServiceResponse> getAllService();
    @Query("SELECT new hbs.booking.model.dto.response.service.customer.ServiceResponse(s.id, s.serviceName, s.image)\n" +
            "FROM Service s\n" +
            "WHERE s.id NOT IN (\n" +
            "    SELECT srv.id\n" +
            "    FROM Service srv\n" +
    ...
    @Query("SELECT new hbs.booking.model.dto.response.service.manager.ServiceDetailResponse(" +
    ...
    List<hbs.booking.model.dto.response.service.manager.ServiceDetailResponse> getServicesByManagerEmail(String managerEmail);
    @Query("SELECT new hbs.booking.model.dto.response.service.admin.ServiceDetailResponse(" +
            "s.id, s.serviceName, s.image, s.description ,s.status) FROM Service s where s.status != 'DELETED'")
    List<hbs.booking.model.dto.response.service.admin.ServiceDetailResponse> getAllServices();
    @Query("SELECT new hbs.booking.model.dto.response.service.manager.ServiceDetailResponse(" +
            "s.id, hsv.id, s.serviceName, s.image, hsv.serviceDescription, hsv.status) FROM Service s " +
            "join s.householdServices hsv join hsv.household h " +
            "WHERE h.id = :householdId and hsv.status != 'DELETED'")
    List<hbs.booking.model.dto.response.service.manager.ServiceDetailResponse> getServicesByHouseholdId(Long householdId);
    @Query("SELECT new hbs.booking.model.dto.response.service.customer.HouseholdByServiceResponse(" +
    ...
    @Query("SELECT new hbs.booking.model.dto.response.service.customer.ServiceDetailResponse(s.id, s.serviceName, s.image) FROM Service s WHERE s.id = :serviceId")
    ...
    @Query("SELECT new hbs.booking.model.dto.response.service.customer.ServiceIntroductionDto(" +
